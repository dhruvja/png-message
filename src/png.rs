...
impl Png {
    ...
    fn remove_chunk(&mut self, chunk_type: &str) -> Result<Chunk> {
        let index = self
            .chunks
            .iter()
            .position(|x| x.chunk_type.chunk == chunk_type.as_bytes());

        match index {
            Some(T) => Ok(self.chunks.remove(T)),
            None => Err("Cannot find chunk type".into()),
        }
    }
    fn header(&self) -> &[u8; 10] {
        Png::STANDARD_HEADER.try_into().unwrap()
    }
    ...
    fn chunk_by_type(&self, chunk_type: &str) -> Option<&Chunk> {
        self.chunks
            .iter()
            .find(|x| x.chunk_type.chunk == chunk_type.as_bytes())
    }
...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(value: &[u8]) -> Result<Self> {
        // Check whether header is matching
        let header = &value[..8];
